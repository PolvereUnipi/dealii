...
void
solve_and_check(const TrilinosWrappers::SparseMatrix &M,
                ...
                const TrilinosWrappers::MPI::Vector  &solution)
{
  ...
  solver.initialize(M);
  TrilinosWrappers::MPI::Vector dst(rhs);
  ...
  for (unsigned int i = 0; i < 3; ++i)
    {
      TrilinosWrappers::MPI::Vector solution(dof_handler.locally_owned_dofs(),
                                             mpi_communicator);
      TrilinosWrappers::MPI::Vector x(dof_handler.locally_owned_dofs(),
                                      mpi_communicator);
      TrilinosWrappers::MPI::Vector b(dof_handler.locally_owned_dofs(),
                                      mpi_communicator);
